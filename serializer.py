...
def serialize_df_to_df(df, suffix, label_col, include_labels=True, descr='Converting to Ditto format'):
	...
	for left_row, right_row, label in tqdm(
		...
		total=len(left_df), desc=descr, unit='rows'
	):
...
def serialize_dfs(left_df, right_df, labels, include_labels=True, descr='Converting to Ditto format'):
	...
	return "\r\n".join([
			"\t".join([
				" ".join([f"COL {col} VAL {left_row[1][col]}" for col in left_cols]),
				" ".join([f"COL {col} VAL {right_row[1][col]}" for col in right_cols]),
				str(label)
			] if include_labels else [
				" ".join([f"COL {col} VAL {left_row[1][col]}" for col in left_cols]),
				" ".join([f"COL {col} VAL {right_row[1][col]}" for col in right_cols])
			])
			for left_row, right_row, label in tqdm(zip(left_df.iterrows(), right_df.iterrows(), labels), total=len(left_df), desc=descr, unit='rows')
		])
...
def split_df(df, suffix, label_col, verbose=False):
	...
	left_df = df.drop(columns=right_df.columns, axis=COLS).drop(columns=[label_col], axis=COLS)
	if verbose:
		print('left_df:')
		print(left_df)
		print('----------'*4,'\n','right_df:')
...
def serialize_df(df, suffix, label_col, verbose=False, include_labels=True, descr='Converting to Ditto format'):
...
def restring_list_col(df, col_name):
	...
	print(df)
	df[col_name] = [[] if item == '[]' else [subitem[col_name] for subitem in ast.literal_eval(item)] for item in tqdm(df[col_name])]
	return df

def binarize_string_binary_col(df, col_name):
	print('Binarizing the string binary columns...')
	print(df)
	df[col_name] = [1 if item.lower() in ['true', '1', 'y', 'yes', 'label_1'] else 0 for item in tqdm(df[col_name])]
	return df

def deserialize_df(df_raw):
	def parse_side(text, suffix=""):
		pattern = re.compile(r"COL (.+?) VAL (.*?) (?=COL|$)")
	...
	parsed_rows = [parse_row(row) for _, row in tqdm(df_raw.iterrows())]
...
if __name__ == "__main__":
	parser = argparse.ArgumentParser(description='Serialize a .csv file into the way Ditto expects it. Writes output as UTF-8.')
	parser.add_argument('target', type=str, help='The target file to process.')
	parser.add_argument('--label-col', type=str, default='label', help='The column that holds the match/nonmatch label. Defaults to "label". Should be a 0 or a 1, but the script will convert bools as well.')
	parser.add_argument('--suffix', type=str, default='_right', help='The suffix on the right side of the pair. Defaults to "_right".')
	parser.add_argument('-v', '--verbose', action='store_true', help='Enable verbose output.')
	parser.add_argument('-f', '--force', action='store_true', help='Force overwrite of the output file if it already exists.')
