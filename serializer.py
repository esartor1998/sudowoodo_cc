...
def serialize_df_to_df(
    df, suffix, label_col, include_labels=True, descr="Converting to Ditto format"
):
...
def serialize_dfs(
    left_df, right_df, labels, include_labels=True, descr="Converting to Ditto format"
):
    left_cols = left_df.columns
    right_cols = right_df.columns
    return "\r\n".join(
...
if __name__ == "__main__":
    ...
    with open(out_location, "w", encoding="utf-8") as f:
        f.write(
            serialize_df(
                pd.read_csv(target_path), args.suffix, args.label_col, args.verbose
            )
        )
    exit(0)
